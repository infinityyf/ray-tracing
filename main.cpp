...
//设置相机位置
EnvironmentLight Light0 = EnvironmentLight(Vector3f(1.f, 1.f, 1.f),0.1);
//设置环境光

PointLight Light1 = PointLight(Vector3f(0,0,150),Vector3f(1.f,1.f,1.f),0.0001f,0.00001f);
//设置点光源

DirectLight Light2 = DirectLight(Vector3f(1.f, 1.f, 1.f),Vector3f::Normalize(Vector3f(1.f, 1.f, -1.f)),0.8f);
//设置平行光
						...
						LPARAM lParam)
{
	...
	*/
	...
	case WM_PAINT:
		...
		//首先初始化场景
		SetObject(Vector3f(0 ,0, 0),100,     Vector3f(0.f,    0.74f,  1.f),lambert);
		SetObject(Vector3f(200, 0, 0), 100,  Vector3f(1.f,    0.f,    1.f),phong);
		SetObject(Vector3f(-200, 0, 0), 100, Vector3f(0.023f, 0.701f, 0.44f),lambert);
		SetObject(Vector3f(0, 0, -10000), 9900 , Vector3f(0.254f,0.411f,1.f),lambert);
		SetObject(Vector3f(0, 1000, 1000), 1000, Vector3f(1.f, 0.84f, 0.f), lambert);
		//初始化背景
		InitScreen();

		//开始扫描，传入每个像素的坐标和相机坐标
		//Ray::scanScreen(MainCamera, ScreenColorBuffer, THREE_SAMPLE);
		Ray::scanScreen(MainCamera, ScreenColorBuffer,THREE_SAMPLE,Light0,Light1,Light2);
		//依次为环境光点光源
